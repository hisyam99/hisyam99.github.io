 ...
 */
export const getProjects = server$(async (
  ...
  try {
    ...
    if (!result?.projects) {
      console.error('❌ Invalid projects response structure. Expected result.projects, got:', result)
 ...
 * Server-side GraphQL query using Qwik server$
 */
export const getProjectById = server$(async (id: string): Promise<Project | null> => {
  const client = createGraphQLClient()
 ...
 * Server-side GraphQL query using Qwik server$
 */
export const getProjectsByUser = server$(async (
  userId: string,
  pagination: PaginationInput = {}
): Promise<PaginatedResponse<Project>> => {
  const client = createGraphQLClient()
  ...
  try {
    ...
    if (result?.errors?.length) {
      throw new Error(result.errors[0].message || 'Failed to fetch projects by user')
    }
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to fetch projects by user')
  }
})
 ...
 */
export const createProject = server$(async (
  token: string,
 ...
 * Server-side GraphQL mutation using Qwik server$
 */
export const updateProject = server$(async (
  token: string,
  id: string,
 ...
 * Server-side GraphQL mutation using Qwik server$
 */
export const deleteProject = server$(async (
  token: string,
  id: string
): Promise<boolean> => {
  const client = createAuthenticatedClient(token)
