 ...
 * Server-side GraphQL query using Qwik server$
 */
export const getPublishedBlogs = server$(async (
  pagination: PaginationInput = {}
): Promise<PaginatedResponse<Blog>> => {
  const client = createGraphQLClient()
  ...
  try {
    ...
    console.log('📊 Published blogs GraphQL result:', JSON.stringify(result, null, 2))

    if (result?.errors?.length) {
      console.error('❌ GraphQL errors:', result.errors)
      throw new Error(result.errors[0].message || 'Failed to fetch published blogs')
    }

    // Graffle returns the data directly, not wrapped in a "data" property
    if (!result?.publishedBlogs) {
      console.error('❌ Invalid response structure. Expected result.publishedBlogs, got:', {
        hasResult: !!result,
 ...
 * Server-side GraphQL query using Qwik server$
 */
export const getBlogBySlug = server$(async (slug: string): Promise<Blog | null> => {
  const client = createGraphQLClient()
 ...
 * Server-side GraphQL query using Qwik server$
 */
export const getBlogsByStatus = server$(async (
  status: 'DRAFT' | 'PUBLISHED',
  pagination: PaginationInput = {}
): Promise<PaginatedResponse<Blog>> => {
  const client = createGraphQLClient()
  ...
  try {
    ...
    if (result?.errors?.length) {
      throw new Error(result.errors[0].message || 'Failed to fetch blogs by status')
    }
    ...
    throw new Error(error instanceof Error ? error.message : 'Failed to fetch blogs by status')
  }
})
