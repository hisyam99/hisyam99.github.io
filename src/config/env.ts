...
export const config = {
  ...
  // API configuration
  api: {
    graphqlEndpoint: getEnvVar('VITE_GRAPHQL_ENDPOINT') || 'http://localhost:4001/graphql',
    timeout: parseInt(getEnvVar('VITE_API_TIMEOUT') || '10000', 10),
  },

  // Development settings
  dev: {
    enableLogging: getEnvVar('VITE_ENABLE_LOGGING') === 'true' || isDevelopment(),
    enableDevtools: getEnvVar('VITE_ENABLE_DEVTOOLS') === 'true' || isDevelopment(),
  },
...
export const logger = {
  ...
  graphql: (operation: string, variables?: unknown, result?: unknown, error?: unknown) => {
    if (config.dev.enableLogging) {
...
export const errorHandler = {
  ...
  graphql: (error: unknown, operation?: string) => {
    ...
    const errorObj = error as { networkError?: unknown; graphQLErrors?: { message: string }[] }
